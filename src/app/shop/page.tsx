...
// For quick fix: define a minimal CartItem type
...
export default function Shop() {
    ...
    // Type-safe cart context
    const cartContext = useCart();
    const cart: CartItem[] = cartContext.cart as CartItem[];
    const addToCart = cartContext.addToCart as (product: Product) => void;
    const removeFromCart = cartContext.removeFromCart as (id: number | string) => void;
                                                ...
                                                <img
                                                    ...
                                                    src={
                                                        product.image_url?.startsWith("http")
                                                            ? product.image_url
                                                            : `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/product-images/${product.image_url}` ?? "/placeholder.png"
                                                    }
                                                />
                                            </div>
                                            <div className="font-semibold text-[#242427] mb-2">{product.name}</div>
                                            <div className="flex items-center justify-between mt-auto">
                                                <span className="text-[#242427] font-semibold">${product.price}</span>
                                                <button
